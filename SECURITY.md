...
Security fixes are applied to the **default branch** of this repository. Image pins live in `compose.yaml` as tag plus digest and are updated by Renovate pull requests; always use current pins or your own vetted versions in production.

This stack also depends on LiteLLM. LiteLLM had a public PyPI supply-chain compromise in releases `1.82.7` and `1.82.8` in March 2026. The pinned image is a later release built from a clean tree, but anyone overriding the LiteLLM pin or installing LiteLLM separately should verify package provenance and avoid the affected versions.
...
Operational guidance (bind address, secrets, first login) lives in [docs/operations/ingress.md](docs/operations/ingress.md) and [docs/operations/backup.md](docs/operations/backup.md).
